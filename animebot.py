...
API_TOKEN = '1876270806:AAE2HGOU3gUda81iyfcHquu4Kub3wzeSXcE'
#----------------------------------
...
async def idd(message, tags=None):
	...
	try:
		...
		photo.close()
		os.remove('tmp/CoffinXanimeBot_' + str(randomint) + ".jpg")
	except Exception:
		traceback.print_exc()
...
async def parser(message, tags, pages, chat_id, info=None, ch_id=None): #Usual parser for usual commands
	...
	try:
		...
		traceback.print_exc()
		await message.reply('𝙾𝙾𝙿𝚂..... 𝚂𝙾𝙼𝙴𝚃𝙷𝙸𝙽𝙶 𝚆𝙴𝙽𝚃 𝚆𝚁𝙾𝙽𝙶!!! 𝙿𝙻𝚂 𝚁𝙴 𝙴𝙽𝚃𝙴𝚁 𝚃𝙷𝙰𝚃 𝙲𝙾𝙼𝙼𝙰𝙽𝙳!')
		try:
			os.remove('tmp/CoffinXanimeBot_' + str(randomint) + ".jpg")
		except:
			pass
...
async def inline_function(inline_query: InlineQuery):
	...
	for post in posts:
		inlinequery.append(InlineQueryResultPhoto(
				...
				caption='<a href="https://t.me/CoffinXanimeBot?start=%s">Download</a>' % (post['id']), 
